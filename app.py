...
# ---- Defaults ----
default_categories = [
    {"Category": "Apparel",     "Mix %": 50.0, "Avg Sale": 6.50, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "Wares",       "Mix %": 23.0, "Avg Sale": 5.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "Shoes",       "Mix %": 7.0,  "Avg Sale": 9.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "Accessories", "Mix %": 7.0,  "Avg Sale": 4.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "E&M",         "Mix %": 5.0,  "Avg Sale": 18.00,"Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "Media",       "Mix %": 2.0,  "Avg Sale": 3.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
    {"Category": "Furniture",   "Mix %": 4.0,  "Avg Sale": 55.00,"Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0, "Pieces Δ%": 0.0},
]
...
st.caption("Enter your target + category metrics. See required **daily pieces**. Use **What‑If** (ASP Δ%, ST Δ%, Pieces Δ%) to model upside and the Assistant Manager bonus (2% of incremental revenue).")

# ---- Category editor ----
st.subheader("Category assumptions (edit these)")
df = pd.DataFrame(default_categories)
edited = st.data_editor(
    df[["Category","Mix %","Avg Sale","Sell-through %","ASP Δ%","ST Δ%","Pieces Δ%"]],
    ...
    column_config={
        ...
        "Avg Sale": st.column_config.NumberColumn(min_value=0.01, step=0.01, format="%.2f"),
        "Sell-through %": st.column_config.NumberColumn(min_value=0.0, max_value=100.0, step=0.1),
        "ASP Δ%": st.column_config.NumberColumn(min_value=-100.0, max_value=500.0, step=1.0, help="Percent change to Avg Sale Price for What‑If"),
        "ST Δ%": st.column_config.NumberColumn(min_value=-90.0,  max_value=500.0, step=1.0, help="Percent change to Sell‑Through for What‑If"),
        "Pieces Δ%": st.column_config.NumberColumn(min_value=-90.0, max_value=500.0, step=1.0, help="Percent change to pieces PRODUCED for What‑If"),
    }
).copy()
...
# What‑If (ASP, ST, and PIECES change)
...
calc["pieces_up_f"] = (1 + (calc["Pieces Δ%"].fillna(0.0) / 100.0)).clip(lower=0.0, upper=10.0)

calc["Req Produced Units (What‑If)"] = calc["Req Produced Units"] * calc["pieces_up_f"]
calc["Daily Required (What‑If)"]     = calc["Req Produced Units (What‑If)"] / days_in_month

calc["sold_baseline"]    = calc["Req Produced Units"] * calc["st"]
calc["sold_whatif"]      = calc["Req Produced Units (What‑If)"] * calc["st_up"]
calc["Revenue Baseline"] = calc["sold_baseline"] * calc["asp"]
...
show_cols = [
    ...
    "ASP Δ%","ST Δ%","Pieces Δ%",
    "Req Produced Units (What‑If)","Daily Required (What‑If)","Revenue What‑If","Incremental Revenue"
]
pretty = calc[show_cols].copy()

pretty["Avg Sale"] = pretty["Avg Sale"].map(lambda x: f"{x:,.2f}")
for c in ["Target Revenue","Revenue What‑If","Incremental Revenue"]:
    pretty[c] = pretty[c].map(lambda x: f"${x:,.0f}")
for c in ["Req Sold Units","Req Produced Units","Req Produced Units (What‑If)"]:
    pretty[c] = pretty[c].round(0).astype(int).astype(str)
pretty["Daily Required Pieces"] = pretty["Daily Required Pieces"].round(1)
pretty["Daily Required (What‑If)"] = pretty["Daily Required (What‑If)"].round(1)
