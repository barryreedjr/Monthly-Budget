...
# -----------------------
DEFAULT_ROWS = [
    {"key": "apparel",     "Category": "Apparel",     "Mix %": 50.0, "Avg Sale (ASP)": 6.50, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "wares",       "Category": "Wares",       "Mix %": 23.0, "Avg Sale (ASP)": 5.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "shoes",       "Category": "Shoes",       "Mix %": 7.0,  "Avg Sale (ASP)": 9.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "accessories", "Category": "Accessories", "Mix %": 7.0,  "Avg Sale (ASP)": 4.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "em",          "Category": "E&M",         "Mix %": 5.0,  "Avg Sale (ASP)": 18.0, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "media",       "Category": "Media",       "Mix %": 2.0,  "Avg Sale (ASP)": 3.00, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
    {"key": "furniture",   "Category": "Furniture",   "Mix %": 4.0,  "Avg Sale (ASP)": 55.0, "Sell-through %": 0.0, "ASP Δ%": 0.0, "ST Δ%": 0.0},
]
...
st.sidebar.caption(f"Labor dollars: **{currency0(monthly_budget * labor_pct/100)}**")
...
edited = st.data_editor(
    pd.DataFrame(DEFAULT_ROWS)[["Category","Mix %","Avg Sale (ASP)","Sell-through %","ASP Δ%","ST Δ%"]],
    num_rows="fixed",
    use_container_width=True,
    hide_index=True,
    column_config={
        "Mix %": st.column_config.NumberColumn(min_value=0.0, max_value=100.0, step=0.1),
        "Avg Sale (ASP)": st.column_config.NumberColumn(min_value=0.01, step=0.1, format="%.2f"),
...
# Keep a copy of original Mix for display; optionally normalize for math
...
calc["asp"] = calc["Avg Sale (ASP)"].astype(float).clip(lower=0.01)
...
# Totals
totals = {
    "Target Revenue": float(calc["Target Revenue"].sum()),
    "Req Sold Units": float(calc["Req Sold Units"].sum()),
    "Req Produced Units": float(calc["Req Produced Units"].sum()),
    "Daily Required Pieces": float(calc["Daily Required Pieces"].sum()),
    "Revenue What-If": float(calc["Revenue What-If"].sum()),
    "Incremental Revenue": float(calc["Incremental Revenue"].sum()),
}
asst_mgr_bonus = totals["Incremental Revenue"] * 0.02  # 2%
...
show_cols = [
    "Category","Mix %","Avg Sale (ASP)","Sell-through %",
...
for c in ["Target Revenue","Revenue What-If","Incremental Revenue"]:
    pretty[c] = pretty[c].apply(currency0)
for c in ["Req Sold Units","Req Produced Units"]:
...
with c1:
    st.metric("Daily Production Needed (pcs/day)", f"{totals['Daily Required Pieces']:.1f}")
with c2:
    st.metric("Labor Allocation (FYI)", currency0(monthly_budget * labor_pct/100))
with c3:
    st.metric("What-If: Assistant Manager Bonus (2%)", currency0(asst_mgr_bonus))
...
st.markdown("""
1) **Target revenue per category** = Monthly Target × Mix %.  
2) **Required sold units** = Target Revenue ÷ Avg Sale Price (ASP).  
3) **Required produced units** = Required Sold ÷ Sell-through.  
4) **Daily required pieces** = Required Produced ÷ Days in Month.  
5) **What-If** holds daily pieces constant and applies ASP/ST deltas to compute incremental revenue and **2%** bonus.
""")
